...
try_from_enum! {
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, EnumIter)]
    enum ChaoAnimalPartForehead: u8 {
        Penguin = 0,
        Gorilla = 8,
        Peacock = 9,
        Parrot = 10,
...
fn chao_char_to_byte(c: char) -> Option<u8> {
    Some(match c {
...
fn chao_byte_to_char(b: u8) -> Option<char> {
    Some(match b {
        0x01 => '!', 0x02 => '\"', 0x03 => '#', 0x04 => '$', 0x05 => '%', 0x06 => '&', 0x07 => '\\', 0x08 => '(',
...
lazy_static! {
    static ref CHAO_STRING_MAPPINGS: BiMap::<char, u8> = {
